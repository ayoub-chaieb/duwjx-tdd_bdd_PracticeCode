...
def step_impl(context, element_name, text_string):
    element_name = "pet_" + element_name.lower().replace(" ", "_")
    element = context.driver.find_element(By.ID, element_name)
    element.clear()
    element.send_keys(text_string)

@when('I click the "{buttom}" button')
def step_impl(context, buttom):
    button_id = buttom.lower().replace(" ", "_") + "-btn"
